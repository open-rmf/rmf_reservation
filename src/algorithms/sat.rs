use std::{collections::{HashMap, VecDeque, HashSet}, hash::Hash, sync::Arc};

use chrono::{Duration, DateTime, Utc, TimeZone};
use itertools::Itertools;
use varisat::{ExtendFormula, Lit, Solver};

use crate::{ReservationRequest, cost_function::{static_cost::StaticCost, self}};
...
impl SATSolver {
    ...
    fn from_hill_climber_with_optimality_proof(problem: SparseScheduleConflictHillClimber) {
        let conflicts = problem.get_banned_reservation_combinations();
        let score_cache = problem.score_cache();
        let requests = problem.literals();
...
fn generate_sat_devil(n_resources: usize, n_alt: usize) -> (Vec<Vec<ReservationRequest>>, Vec<String>) {
